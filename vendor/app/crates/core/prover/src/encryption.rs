...
//!                   │          └── BN254 Note Private Key → Poseidon2(dom=3) → Note Public Key
//!                   │                                     └─ Poseidon2(dom=5) → Nullifier Key (nk)
//!                   │
//!                   └── SHA-256("privacy-pool/encryption-key/v2" || sig)
//!                              └── X25519 Encryption Keypair
//! ```
//!
//! The nullifier key (`nk`) is derived from the note private key the same way
//! the note public key is (Poseidon2, not a new SHA domain) — see the
//! Zcash-style nullifier-key split: `nk` lets its holder compute nullifiers
//! for owned notes without granting spend authority, which stays with the
//! note private key.
...
//! think)
use crate::crypto::{derive_nullifier_key, derive_public_key};
use alloc::vec::Vec;
...
/// Keypairs derivation
///
/// Returns `(note_keypair, encryption_keypair, nullifier_key)`. `nullifier_key`
/// (`nk`) is the LE-encoded 32-byte Zcash-style nullifier key derived from the
/// note private key (same convention as `note_keypair.private`).
pub fn derive_encryption_and_note_keypairs(
    signature: KeyDerivationSignature,
) -> Result<(NoteKeyPair, EncryptionKeyPair, Vec<u8>)> {
    ...
    let nullifier_key = derive_nullifier_key(&note_private_key.0)?;
    ...
    Ok((note_keypair, encryption_keypair, nullifier_key))
}
