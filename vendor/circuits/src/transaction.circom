...
template Transaction(levels, nIns, nOuts) {
    ...
    for (var tx = 0; tx < nIns; tx++) {
        ...
        // Derives the nullifier key from the spend private key (Zcash-style split)
        inNullifierKey[tx] = NullifierKey();
        inNullifierKey[tx].privateKey <== inPrivateKey[tx];

        // Computes the signature as hash(nullifierKey, commitment, merklePath)
        inSignature[tx] = Signature();
        inSignature[tx].privateKey <== inNullifierKey[tx].nk;
