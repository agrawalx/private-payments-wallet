...
//!
//! Transpiles the Circom witness-generator WASM (`circuits/policy_tx_2_2.wasm`)
...
//! `policy_tx_2_2_witness()` function with no WASM interpreter/JIT, so it works
//! on `aarch64-linux-android`.
//!
//! `transpile_wasm` scans the given directory for `*.wasm` files and emits a
//! `<stem>_witness` function for each (here: `policy_tx_2_2`).

fn main() {
    // NOTE: the wasm is named `policytx22.wasm` (no underscores) on purpose —
    ...
    // policytx22 + selectiveDisclosure1.
    println!("cargo:rerun-if-changed=circuits/policytx22.wasm");
